 ...
 *  - Block F3/F5; F12 / Ctrl+Shift+I pass through to the WebView's native
 *    devtools accelerator (tauri.conf.json sets `devtools: true` and the
 *    `devtools` Cargo feature keeps them available in release builds)
...
export function registerGlobalUX(app: App) {
  const handler = (e: KeyboardEvent) => {
    ...
    // preventDefault here.
    if (e.key === "F12" || (e.shiftKey && (e.ctrlKey || e.metaKey) && !e.altKey && e.key.toLowerCase() === "i")) {
      return;
    }
    ...
    if (ctrl && !e.altKey) {
      ...
      // remaining Ctrl+Shift combos (J/C console) are blocked like before
      if (e.shiftKey && ["j", "c"].includes(k)) {
        e.preventDefault();
        return;
      }
    }
