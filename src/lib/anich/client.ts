...
import { decodePlaybackURL } from "./decode";

// ── Anich SDK parsers live in this app now ──
// AiKF previously delegated VOD / danmaku / bangumi-list /
// playback-URL decoding to the `Artplayer.anich` namespace shipped
// inside a customized ArtPlayer bundle. The player is now the stock
// (unmodified) ArtPlayer, so the data layer is fully self-contained:
// `./parsers` holds the protobuf decoders and `./decode` the playback
// URL decoder — both ported from the anichsdk Go source.
import type {
  BangumiCalendar,
 ...
 * `decodePlaybackURL` lives in `./decode` (ported from anichsdk Go
 * source). We re-export it here so existing call sites that import
 * from this module keep working.
 */
export { decodePlaybackURL };

/** Generic external-host GET (bytes) — used by the danmaku module for
 * Bilibili / Dandanplay proxies that live outside ANICH_BASE_URL. */
export async function fetchExternalBytes(url: string): Promise<Uint8Array> {
  const { bytes } = await rustFetch(url, "*/*");
  return bytes;
}

/** Generic external-host GET (text) — dandanplay v2 endpoints are JSON. */
export async function fetchExternalText(url: string): Promise<string> {
  const { text } = await rustFetch(url, "application/json");
  return text;
}
...
// `parseDanmakuPage` comes from the local `./parsers` protobuf
// decoder — see the imports at the top of this file.
