...
// Trigger on active.list.length + sidebar state so grid recomputes when day
// changes OR sidebar toggles (changing main width).
const { containerRef: calGridRef, style: calGridStyle } = useResponsiveGrid({
  minWidth: 280, gap: 12,
  trigger: () => `${active.value?.list.length ?? 0}-${ui.sidebarCollapsed}`,
});
</script>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="no-scrollbar -mx-1 flex gap-1.5 overflow-x-auto px-1">
          <button
            ...
            @click="activeDay = d"
            :class="cn('state-layer relative flex min-w-[68px] flex-col items-center gap-0.5 rounded-lg px-4 py-2.5 text-sm font-semibold transition-colors', activeDay === d ? 'bg-foreground text-background' : 'text-muted-foreground hover:bg-foreground/5 hover:text-foreground')"
          >
    ...
    <section class="flex flex-col gap-3">
      ...
      <div v-else-if="active && active.list.length > 0" ref="calGridRef" class="min-w-0 w-full overflow-hidden" :style="{ ...calGridStyle, contain: 'layout', maxWidth: '100%' }">
        <button
          ...
          class="state-layer surface group flex min-w-0 items-center gap-3 rounded-2xl p-3 text-left transition-colors hover:border-foreground/20 fade-up"
        >
          <CoverImage :src="item.image" :alt="item.title" ratio="portrait" class="h-20 w-14 shrink-0" rounded="rounded-lg" />
          <div class="min-w-0 flex-1">
            <p class="line-clamp-1 text-sm font-semibold text-foreground">{{ item.title }}</p>
            <div class="mt-1 flex flex-wrap gap-1">
              <span v-for="g in item.genres.slice(0, 2)" :key="g" class="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{{ g }}</span>
              <span class="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{{ item.lang }}</span>
            </div>
