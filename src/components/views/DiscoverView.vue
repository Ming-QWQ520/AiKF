...
// NOTE: must be declared AFTER activeDayList / fresh — the watch() inside
// evaluates the trigger getter synchronously, so referencing later-declared
// consts here would throw a TDZ ReferenceError and abort the whole mount.
      ...
      <aside class="hidden w-[240px] shrink-0 flex-col gap-6 xl:flex xl:w-[280px]">
        <div>
          ...
          <div v-else class="space-y-1">
            ...
            >
              <span :class="cn('flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-[11px] font-bold', i < 3 ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground')">{{ i + 1 }}</span>
