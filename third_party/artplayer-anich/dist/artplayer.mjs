...
const scheme = {
  ...
  autoOrientation: b,
  airplay: b,
  // Anich Edition: when `true`, force-disables all built-in control buttons
  // (screenshot/setting/loop/flip/playbackRate/aspectRatio/fullscreen/
  // fullscreenWeb/pip/airplay/fastForward) in one flag. Host apps that
  // supply their own `controls` array (e.g. AiKF) can use this to guarantee
  // a blank-slate control bar instead of setting 11 separate flags.
  blankSlate: b,
  // Anich Edition: when `true`, marks the container with the
  // `art-theme-anich` class so the bundled Anich-flavored rose/Bilibili-style
  // CSS overrides take effect (danmaku-first layout, rose progress bar,
  // glassy semi-transparent control bar).
  anichTheme: b,
  proxy: `?${f}`,
  plugins: [f],
...
function optionInit(art) {
  const {
    option,
    storage,
    template: { $video, $poster, $player }
  } = art;
  if (option.blankSlate) {
    for (const flag of BLANK_SLATE_FLAGS) {
      option[flag] = false;
    }
  }
  if (option.anichTheme) {
    addClass($player, "art-theme-anich");
  }
...
function urlMix(art) {
  ...
  let currentTypeDispose = null;
  function runDispose() {
    if (typeof currentTypeDispose === "function") {
      try {
        currentTypeDispose.call(null, $video, art);
      } catch (err) {
        console.warn("[artplayer:anich] customType dispose threw", err);
      }
      currentTypeDispose = null;
    }
    art.emit("customtype:destroy");
  }
  art.runCustomTypeDispose = runDispose;
  def(art, "url", {
    get() {
      return $video.src;
    },
    async set(newUrl) {
      if (newUrl) {
        ...
        if (typeName && typeCallback) {
          ...
          runDispose();
          art.loading.show = true;
          const maybeDispose = typeCallback.call(art, $video, newUrl, art);
          if (typeof maybeDispose === "function") {
            currentTypeDispose = maybeDispose;
          } else if (maybeDispose && typeof maybeDispose.destroy === "function") {
            currentTypeDispose = () => maybeDispose.destroy();
          }
        } else {
          runDispose();
...
function readVarint(data, offset = 0) {
  ...
  if (index < data.length) {
    ...
    while (index < data.length) {
      const byte = data[index];
      index += 1;
      big |= BigInt(byte & 127) << bShift;
      if ((byte & 128) === 0n)
        break;
      bShift += 7n;
    }
    return { value: Number(big), next: index };
  }
  return { value: result >>> 0, next: index };
}
function normalizeWireData(data) {
  ...
  if (data[0] === 91) {
    try {
      ...
      if (Array.isArray(arr) && arr.every((n2) => typeof n2 === "number" && n2 >= 0 && n2 <= 255)) {
...
function nextField(data, offset = 0) {
  if (offset >= data.length)
    return null;
  const { value: tag, next: tagEnd } = readVarint(data, offset);
  const num = tag >>> 3;
  const wire = tag & 7;
  const next = tagEnd;
  const field = {
  ...
  if (wire === WIRE_VARINT) {
    const { value, next: vEnd } = readVarint(data, next);
    field.varint = value;
    field.next = vEnd;
  } else if (wire === WIRE_FIXED64) {
    const view = new DataView(data.buffer, data.byteOffset + next, 8);
    const lo = view.getUint32(0, true);
    const hi = view.getUint32(4, true);
    field.fixed64 = hi * 4294967296 + lo;
    field.next = next + 8;
  } else if (wire === WIRE_BYTES) {
    const { value: len, next: lenEnd } = readVarint(data, next);
    field.bytes = data.subarray(lenEnd, lenEnd + len);
    field.next = lenEnd + len;
  } else if (wire === WIRE_FIXED32) {
    const view = new DataView(data.buffer, data.byteOffset + next, 4);
    field.fixed32 = view.getUint32(0, true);
    field.next = next + 4;
  } else {
    return null;
  }
...
function* iterateFields(data) {
  let offset = 0;
  while (offset < data.length) {
    const field = nextField(data, offset);
    if (!field)
      break;
    yield field;
    offset = field.next;
...
function fieldString(field) {
  if (!field || field.wire !== WIRE_BYTES || !field.bytes)
    return null;
  return new TextDecoder().decode(field.bytes);
}
function fieldDouble(field) {
  if (!field || field.wire !== WIRE_BYTES || !field.bytes || field.bytes.length !== 8)
    return null;
  const view = new DataView(field.bytes.buffer, field.bytes.byteOffset, 8);
  return view.getFloat64(0, true);
}
function fieldVarintNum(field) {
  if (!field || field.wire !== WIRE_VARINT)
    return null;
  return field.varint;
}
function fieldVarintBool(field) {
  const v = fieldVarintNum(field);
  if (v === null)
    return null;
  return v !== 0;
}
...
function parsePlaybackSource(data) {
  const item = { rawURL: "", url: "", sort: 0, type: "", caption: "" };
  for (const f2 of iterateFields(data)) {
    switch (f2.num) {
      case 1: {
        const v = fieldString(f2);
        if (v != null)
          item.rawURL = v;
        break;
      }
...
function parseVOD(data) {
  const bytes = normalizeWireData(data);
  const sources = [];
  for (const f2 of iterateFields(bytes)) {
    if (f2.num === 1 && f2.wire === 2 && f2.bytes) {
      sources.push(parsePlaybackSource(f2.bytes));
    }
  }
  return { sources };
}
function parseDanmakuItem(data) {
  const item = {
    id: "",
    color: "",
    date: 0,
...
function parseDanmakuPage(data) {
  const bytes = normalizeWireData(data);
  const page = { items: [], skip: 0 };
  for (const f2 of iterateFields(bytes)) {
    if (f2.num === 1 && f2.wire === 2 && f2.bytes) {
      page.items.push(parseDanmakuItem(f2.bytes));
    } else if (f2.num === 2 && f2.wire === 0) {
      page.skip = fieldVarintNum(f2) ?? 0;
    }
  }
  return page;
}
function parseBilibiliDanmakuItem(data) {
  const item = {
    id: 0,
    progress: 0,
    mode: 0,
  ...
  for (const f2 of iterateFields(data)) {
    switch (f2.num) {
      case 1:
        if (f2.wire === 0)
          item.id = fieldVarintNum(f2) ?? 0;
        break;
      case 2:
        if (f2.wire === 0)
          item.progress = fieldVarintNum(f2) ?? 0;
        break;
      case 3:
        if (f2.wire === 0)
          item.mode = fieldVarintNum(f2) ?? 0;
        break;
      case 4:
        if (f2.wire === 0)
          item.fontSize = fieldVarintNum(f2) ?? 0;
        break;
      case 5:
        if (f2.wire === 0)
          item.color = fieldVarintNum(f2) ?? 0;
        break;
      case 6: {
        const v = fieldString(f2);
        if (v != null)
      ...
      case 8:
        if (f2.wire === 0)
          item.ctime = fieldVarintNum(f2) ?? 0;
        break;
      case 9:
        if (f2.wire === 0)
          item.weight = fieldVarintNum(f2) ?? 0;
        break;
      case 10: {
        const v = fieldString(f2);
        if (v != null)
...
function parseBilibiliDanmakuSegment(data) {
  const bytes = normalizeWireData(data);
  const segment = { items: [] };
  for (const f2 of iterateFields(bytes)) {
    if (f2.num === 1 && f2.wire === 2 && f2.bytes) {
      segment.items.push(parseBilibiliDanmakuItem(f2.bytes));
    }
  }
  return segment;
}
function parseBangumiListItem(data) {
  const item = {
    id: 0,
    title: "",
    episode: 0,
  ...
  for (const f2 of iterateFields(data)) {
    switch (f2.num) {
      ...
      case 3:
        if (f2.wire === 0)
          item.episode = fieldVarintNum(f2) ?? 0;
        break;
      case 4:
        if (f2.wire === 0)
          item.episodesTotal = fieldVarintNum(f2) ?? 0;
        break;
      case 5: {
        const v = fieldString(f2);
        if (v != null)
...
function parseBangumiList(data) {
  const bytes = normalizeWireData(data);
  const out = { items: [], prev: 0, next: 0 };
  for (const f2 of iterateFields(bytes)) {
    switch (f2.num) {
      case 1:
        if (f2.wire === 2 && f2.bytes)
          out.items.push(parseBangumiListItem(f2.bytes));
        break;
      case 2:
        if (f2.wire === 0)
          out.prev = fieldVarintNum(f2) ?? 0;
        break;
      case 3:
        if (f2.wire === 0)
          out.next = fieldVarintNum(f2) ?? 0;
        break;
    }
  }
  return out;
...
function parseComment(raw) {
  return {
    ...
    contents: Array.isArray(raw?.contents) ? raw.contents.map((c) => ({ type: String(c?.type ?? ""), content: String(c?.content ?? "") })) : [],
    user: raw?.user ? {
      name: String(raw.user.name ?? ""),
      exp: Number(raw.user.exp ?? 0),
      avatar: String(raw.user.avatar ?? ""),
      color: String(raw.user.color ?? ""),
      issp: Boolean(raw.user.issp)
    } : null,
    address: String(raw?.address ?? ""),
    like: Boolean(raw?.like),
    likesCount: Number(raw?.likes_count ?? 0),
    repliesCount: Number(raw?.replies_count ?? 0),
    replies: Array.isArray(raw?.replies) ? raw.replies.map(parseComment) : []
  };
}
function parseCommentList(raw) {
  return {
    error: Boolean(raw?.error),
    message: String(raw?.message ?? ""),
    data: Array.isArray(raw?.body?.data) ? raw.body.data.map(parseComment) : []
  };
}
function decodePlaybackURL(raw) {
  if (!raw)
    return "";
  if (raw.startsWith("http://") || raw.startsWith("https://"))
    return raw;
  if (raw.length < 4)
    throw new Error("playback URL token is too short");
  const normalized = raw.slice(0, 3) + raw.slice(4);
  return decodeBase64Flex(normalized);
}
function decodeBase64Flex(s2) {
  const clean = s2.replace(/\s/g, "");
...
const DEFAULT_BASE_URL = "https://anich.sends.eu.org";
const DEFAULT_BILIBILI_DM_URL = "https://dm.bili.i.me.cdn.cloudflare.net";
const DEFAULT_QQ_VIDEO_URL = "https://dm.video.qq.com";
const DEFAULT_DANDAN_URL = "https://dandan.emmmm.eu.org";
const DANDAN_OFFICIAL_URL = "https://api.dandanplay.net";
const DEFAULT_USER_AGENT = "anich Browser 1.0.0";
const DEFAULT_DANMAKU_PAGE_SIZE = 3e3;
function createAnichClient(opts = {}) {
  const baseUrl = (opts.baseUrl || DEFAULT_BASE_URL).replace(/\/$/, "");
  const bilibiliDmUrl = (opts.bilibiliDmUrl || DEFAULT_BILIBILI_DM_URL).replace(/\/$/, "");
  const qqVideoUrl = (opts.qqVideoUrl || DEFAULT_QQ_VIDEO_URL).replace(/\/$/, "");
  const dandanUrl = (opts.dandanUrl || DEFAULT_DANDAN_URL).replace(/\/$/, "");
  const userAgent2 = opts.userAgent || DEFAULT_USER_AGENT;
  ...
  function get2(path, query2, meta = {}) {
    const url = buildUrl(baseUrl, path, query2);
    return request("GET", url, { accept: meta.accept || "*/*", token: meta.token });
  }
  async function getRuntimeConfig() {
    const resp = await fetchImpl(`${baseUrl}/check/api`);
    if (!resp.ok)
      throw new Error(`GET /check/api: ${resp.status}`);
    return resp.json();
  }
  async function getPlaybackSources(bangumiID, episode) {
    if (bangumiID <= 0 || episode <= 0)
      throw new Error("bangumiID and episode must be positive");
    const resp = await get2(`/vod/${bangumiID}/${episode}`);
    if (!resp.ok)
      throw new Error(`GET /vod/${bangumiID}/${episode}: ${resp.status}`);
    const arr = await resp.json();
    const bytes = Array.isArray(arr) ? new Uint8Array(arr) : new Uint8Array();
    return bytes;
  }
  async function getDanmakuPage(bangumiID, episode, skip) {
    if (bangumiID <= 0 || episode <= 0 || skip < 0)
      throw new Error("invalid danmaku parameters");
    const resp = await get2("/danmaku", { bangumi: bangumiID, episode, skip });
    if (!resp.ok)
      throw new Error(`GET /danmaku: ${resp.status}`);
    return new Uint8Array(await resp.arrayBuffer());
  }
  async function getAllDanmaku(bangumiID, episode, pageSize = DEFAULT_DANMAKU_PAGE_SIZE) {
    const items = [];
    for (let skip = 0; ; skip += pageSize) {
      const bytes = await getDanmakuPage(bangumiID, episode, skip);
      const page = parseDanmakuPage(bytes);
      items.push(...page.items);
      if (page.items.length < pageSize)
        return items;
    }
  }
  async function getBilibiliDanmakuSegment(typ, oid, segmentIndex) {
    if (typ <= 0 || oid <= 0 || segmentIndex <= 0)
      throw new Error("invalid bilibili danmaku parameters");
    const url = buildUrl(bilibiliDmUrl, "/", { type: typ, oid, segment_index: segmentIndex });
    const resp = await request("GET", url, { accept: "*/*" });
    if (!resp.ok)
      throw new Error(`GET bili dm: ${resp.status}`);
    return new Uint8Array(await resp.arrayBuffer());
  }
  async function getEpisodeComments(bangumiID, episode, skip = "") {
    const resp = await get2("/comment", {
      type: "bangumi_episode",
      id: `${bangumiID}|${episode}`,
      skip
    }, { accept: "application/json" });
    if (!resp.ok)
      throw new Error(`GET /comment: ${resp.status}`);
    return resp.json();
  }
  async function getEpisodeCommentCount(bangumiID, episode) {
    const resp = await get2("/comment/count", {
      type: "bangumi_episode",
      id: `${bangumiID}|${episode}`
    }, { accept: "application/json" });
    if (!resp.ok)
      throw new Error(`GET /comment/count: ${resp.status}`);
    return resp.json();
  }
  async function getCommentReplies(commentID, skip = "") {
    const resp = await get2(`/comment/${commentID}/replies`, { skip }, { accept: "application/json" });
    if (!resp.ok)
      throw new Error(`GET /comment/${commentID}/replies: ${resp.status}`);
    return resp.json();
  }
  return {
    getRuntimeConfig,
    ...
    getDanmakuPage,
    getAllDanmaku,
    getBilibiliDanmakuSegment,
    getEpisodeComments,
    getEpisodeCommentCount,
    getCommentReplies,
    config: { baseUrl, bilibiliDmUrl, qqVideoUrl, dandanUrl, userAgent: userAgent2, token }
  };
}
function buildUrl(baseURL, path, query2) {
  const u = new URL(`${baseURL.replace(/\/$/, "")}/${path.replace(/^\//, "")}`);
  if (query2) {
...
const anich = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  DANDAN_OFFICIAL_URL,
  DEFAULT_BASE_URL,
  DEFAULT_BILIBILI_DM_URL,
  DEFAULT_DANDAN_URL,
  DEFAULT_DANMAKU_PAGE_SIZE,
  DEFAULT_QQ_VIDEO_URL,
  DEFAULT_USER_AGENT,
  WIRE,
  buildUrl,
  createAnichClient,
  ...
  normalizeWireData,
  parseBangumiList,
  parseBangumiListItem,
  parseBilibiliDanmakuItem,
  parseBilibiliDanmakuSegment,
  parseComment,
  parseCommentList,
  parseDanmakuItem,
  parseDanmakuPage,
...
const ROSE_THEME = "#fb7185";
const ANICH_ICONS = {
  play: '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>',
  pause: '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>',
  volume: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 10v4h4l5 5V5L7 10z" fill="currentColor"/><path d="M16 8a5 5 0 0 1 0 8"/><path d="M19 5a9 9 0 0 1 0 14"/></svg>',
  muted: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 10v4h4l5 5V5L7 10z" fill="currentColor"/><path d="M22 9l-6 6"/><path d="M16 9l6 6"/></svg>',
  next: '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5v14l8-7zM16 5h2v14h-2z"/></svg>',
  danmaku: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M7 10h4M9 14h7"/></svg>',
  fullscreenWeb: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M7 7h4M7 7v4M17 17h-4M17 17v-4"/></svg>',
  fullscreen: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 9V4h5M20 9V4h-5M4 15v5h5M20 15v5h-5"/></svg>',
...
function anichPreset(params = {}) {
  const {
    container,
    url = "",
    type = "",
    m3u8Loader,
    // The danmaku plugin loads items AFTER `art.ready`, so we don't
    // pass these into the option object — host apps can read them
    // back via the returned `_danmakuItems` field.
    danmakuItems: _danmakuItems = [],
    customControls = [],
    customSettings = [],
    anichTheme = true,
    autoplay = false,
  ...
  if (m3u8Loader) {
    customType = {
      m3u8(video, src, art) {
        const handle = m3u8Loader(video, src, art);
        if (typeof handle === "function")
          return handle;
        if (handle && typeof handle.destroy === "function")
          return () => handle.destroy();
        if (handle && handle.hls && typeof handle.hls.destroy === "function") {
          return () => handle.hls.destroy();
        }
        return void 0;
      }
    };
  ...
  return {
    ...
    plugins,
    // Default CSS variables for the Anich rose theme
    cssVar: {
      "--art-theme": ROSE_THEME,
      "--art-progress-played": ROSE_THEME
    },
    // Expose the initial danmaku list back to the host so it can be
    // loaded via `art.danmuku.load(preset._danmakuItems)` once the
    // player is ready. ArtPlayer itself doesn't accept danmaku items
    // directly in the option object — they must go through the plugin.
    _danmakuItems
  };
}
function toDanmukuItems(items) {
  if (!Array.isArray(items))
    return [];
  return items.filter((it) => it && it.text).map((it) => ({
    text: it.text,
    time: typeof it.time === "number" ? it.time : Number(it.time) || 0,
    mode: anichTypeToDanmukuMode(it.type),
    color: anichColorToCss(it.color),
    border: false
  }));
}
function anichTypeToDanmukuMode(type) {
  switch (type) {
    case 1:
      return "top";
    case 2:
...
function anichColorToCss(color) {
  if (!color && color !== 0)
    return "#ffffff";
  ...
  const n2 = Number(color);
  if (!Number.isNaN(n2) && n2 > 0)
    return `#${n2.toString(16).padStart(6, "0")}`;
  return "#ffffff";
}
...
class Artplayer extends Emitter {
  ...
  // Anich Edition: bundled JS port of the Anich Go SDK. Host apps can
  // call `Artplayer.anich.decodePlaybackURL(raw)` etc. instead of
  // shipping their own wire-format / base64 decoder.
  static get anich() {
    return anich;
  }
  // Anich Edition: factory helpers that return ready-to-use option
  // objects (or option fragments) for common host integrations.
  static get presets() {
    return presets;
  }
  static get option() {
    return {
      ...
      airplay: false,
      // Anich Edition: when `true`, force-disables all built-in control
      // buttons (screenshot/setting/loop/flip/playbackRate/aspectRatio/
      // fullscreen/fullscreenWeb/pip/airplay/fastForward) so the host
      // app can supply its own `controls` array on a blank slate.
      blankSlate: false,
      // Anich Edition: when `true`, marks the container with the
      // `art-theme-anich` class so the bundled Anich-flavored rose /
      // Bilibili-style CSS overrides take effect.
      anichTheme: false,
      proxy: void 0,
      layers: [],
      contextmenu: [],
  ...
  destroy(removeHtml = true) {
    if (typeof this.runCustomTypeDispose === "function") {
      try {
        this.runCustomTypeDispose();
      } catch (err) {
        console.warn("[artplayer:anich] customType dispose on destroy threw", err);
      }
    }
