...
export interface Events {
  ...
  /**
   * Anich Edition: emitted when the currently-active `customType` has
   * been torn down (e.g. an attached Hls.js instance has been
   * destroyed). Useful for host apps that need to clean up references
   * kept outside the player instance.
   */
  'customtype:destroy': []

  /**
   * Anich Edition: emitted when the user clicks the "next episode"
   * control built by `Artplayer.presets.anich`. Host apps subscribe
   * and switch the source themselves.
   */
  'anich:next-episode': []
