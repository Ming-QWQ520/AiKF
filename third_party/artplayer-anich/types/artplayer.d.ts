...
import type { AnichNamespace } from './anich'
import type { PresetsNamespace } from './presets'
...
export default class Artplayer extends Player {
   ...
   * call `Artplayer.anich.decodePlaybackURL(raw)`,
   * `Artplayer.anich.parseVOD(bytes)`, etc. instead of shipping their
   * own wire-format / base64 decoder.
   */
  static readonly anich: AnichNamespace

  /**
   * Anich Edition: preset factories. Currently exposes `presets.anich(params)`
   * which returns a ready-to-use ArtPlayer option object pre-configured for
   * the AiKF / Anich-style danmaku-first player shell.
   */
  static readonly presets: PresetsNamespace
